 ...
 */
void drawtext(float xc, float yc, const std::string& str_text, float boundx, float boundy, bool center) {
    ...
    if (!center){
    X11_xmagicoffset = trans_xmult* width;
    X11_ymagicoffset = trans_ymult* height;}
#endif
...
void set_visible_world(float left, float bottom, float right, float top, bool save) {
    set_visible_world(t_bound_box(left, bottom, right, top), save);
}

void set_visible_world(const t_bound_box& bounds, bool save) {
